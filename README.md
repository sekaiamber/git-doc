...
好处（可以不看）：双基础分支的设定由来以久，`develop`分支保证了所有修改只会影响`develop`分支和测试环境，待测试通过才会影响`master`主分支，也可以在`develop`分支渡过数个PR后才合并到`master`，这意味着`develop`分支的迭代远远快于`master`分支，保证了整个项目不会因为未发现的错误影响大局。
...
上述方案涉及到Git流程中的一部分，用心的同学可以看看本项目的分支网络，本项目也遵循上述原则（除了最初几个分支，只是为了展示分支名称），细细体会两个基础分支带来的好处。

Git是个很灵活的工具（个人觉得，比SVN不知道高到哪里去了），根本没有所谓的“最佳实践”，怎么用的舒服就应该怎么用，不过很多约定也能使得使用过程中事半功倍。用好Git，享受轻松快捷的版本控制，do whatever you want using GIT :)